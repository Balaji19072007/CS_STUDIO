...
const Loader = ({ size = 'md', message = 'Loading...', className = '', showText = true }) => {
    ...
    return (
        <div className={`flex flex-col items-center justify-center ${showText ? 'p-8' : ''} text-center text-gray-700 ${className}`}>
            <i
                data-feather="loader"
                className={`${spinnerClass} animate-spin text-primary-600`}
            ></i>
            {showText && <p className="mt-3 text-sm font-medium text-gray-600">{message}</p>}
        </div>
    );
};
